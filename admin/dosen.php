...
if ($_SESSION["status"] == 'login') {
    ...
    $rslt = mysqli_query($mysqli, "SELECT username FROM users WHERE username = '$username'");
    $user = mysqli_fetch_assoc($rslt);
    $data = mysqli_query($mysqli, "SELECT * FROM dosen INNER JOIN users ON dosen.id_users = users.id_users ORDER BY nip ASC");
} else {
...
if (isset($_POST['edit'])) {
    ...
    if (in_array($fileType, $allowedTypes)) {
        ...
        if (move_uploaded_file($fileTemp, $uploadPath)) {
            ...
            $sql = "UPDATE dosen SET id_users='$id_users',nip='$nip',nama_dosen='$nama_dosen',email_dosen='$email_dosen',gambar='$gambar' WHERE id_usere='$id_users'";
            $edit = mysqli_query($mysqli, $sql);
            header("Refresh:0");
        } else {
            $edit = 0;
                                    ...
                                    <tbody>
                                        ...
                                        <?php foreach ($data as $row) : ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="" data-toggle="modal" data-target="#editModal<?php echo $row["id_dosen"] ?>" class="btn btn-sm btn-info text-white"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                                            <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                                            <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z" />
                                                        </svg>
                                                    </a> |
                                                    <a href="./delete/dosen.php?id=<?php echo $row["nip"] ?>" class="btn btn-sm btn-danger" onclick=" return confirm(' Anda yakin ingin menghapusnya')"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
                                            ...
                                            <?php include('../components/edit_modal_dosen.php') ?>
                                            <?php $i++; ?>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
